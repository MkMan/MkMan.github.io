import * as React from 'react';
import { LogoComponentProps } from './logo.model';

export function LogoComponent(props: LogoComponentProps) {
  switch (props.logo) {

    case 'angular':
      return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250" className={props.className}>
          <path fill="#dd0031" d="M125 30L31.9 63.2l14.2 123.1L125 230l78.9-43.7 14.2-123.1z" />
          <path fill="#c3002f" d="M125 30v22.2-.1V230l78.9-43.7 14.2-123.1L125 30z" />
          <path d="M125 52.1L66.8 182.6h21.7l11.7-29.2h49.4l11.7 29.2H183L125 52.1zm17 83.3h-34l17-40.9 17 40.9z" fill="#fff" />
        </svg>
      );

    case 'js':
      return (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 630 630" className={props.className}>
          <path fill="#f7df1e" d="M0 0h630v630H0z" />
          <path d="M165.65 526.474l48.212-29.178c9.302 16.492 17.763 30.444 38.058 30.444 19.454 0 31.719-7.61 31.719-37.21v-201.3h59.203v202.137c0 61.32-35.943 89.232-88.385 89.232-47.36 0-74.852-24.529-88.807-54.13M375 520.13l48.206-27.911c12.69 20.724 29.183 35.947 58.361 35.947 24.532 0 40.174-12.265 40.174-29.181 0-20.299-16.068-27.489-43.135-39.326l-14.797-6.348c-42.715-18.181-71.05-41.017-71.05-89.227 0-44.404 33.831-78.238 86.695-78.238 37.637 0 64.702 13.111 84.154 47.366l-46.097 29.602c-10.15-18.183-21.142-25.372-38.057-25.372-17.339 0-28.335 10.995-28.335 25.372 0 17.762 10.996 24.952 36.367 35.948l14.8 6.343c50.325 21.569 78.66 43.557 78.66 93.034 0 53.287-41.866 82.465-98.11 82.465-54.976 0-90.5-26.218-107.836-60.474" />
        </svg>
      );

    default:
      return;
  }
}
